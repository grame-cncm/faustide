...
(function(mod) {
    ...
    CodeMirror.defineMode("clike", function(config, parserConfig) {
        var indentUnit = config.indentUnit,
            ...
            indentStatements = parserConfig.indentStatements !==
            false,
        ...
        function tokenBase(stream, state) {
            ...
            if (isOperatorChar.test(ch)) {
                while (!stream.match(/^\/[\/*]/, false) && stream.eat(
                        isOperatorChar)) {}
                return "operator";
            }
            ...
            if (contains(keywords, cur)) {
                if (contains(blockKeywords, cur)) curPunc =
                    "newstatement";
        ...
        return {
            ...
            token: function(stream, state) {
                ...
                var style = (state.tokenize || tokenBase)(
                    stream, state);
                ...
                if (hooks.token) {
                    var result = hooks.token(stream, state,
                        style);
            ...
            indent: function(state, textAfter) {
                if (state.tokenize != tokenBase && state.tokenize !=
                    null || state.typeAtEndOfLine) return
                    CodeMirror.Pass;
                var ctx = state.context,
                ...
                if (parserConfig.dontIndentStatements)
                    while (ctx.type == "statement" &&
                        parserConfig.dontIndentStatements.test(
                            ctx.info))
                        ctx = ctx.prev
                if (hooks.indent) {
                    var hook = hooks.indent(state, ctx,
                        textAfter);
                ...
                var switchBlock = ctx.prev && ctx.prev.info ==
                    "switch";
                if (parserConfig.allmanIndentation && /[{(]/.test(
                        firstChar)) {
    ...
    def("text/x-java", {
        ...
        defKeywords: words(
            "class interface package enum @interface"),
        typeFirstDefinitions: true,
        atoms: words("true false null"),
        ...
        hooks: {
            "@": function(stream) {
                ...
                if (stream.match('interface', false)) return
                    false;
        ...
        hooks: {
            ...
            "=": function(stream, state) {
                ...
                if (cx.type == "}" && cx.align && stream.eat(
                        ">")) {
                    state.context = new Context(cx.indented, cx
                        .column, cx.type, cx.info, null, cx
                        .prev)
                    return "operator"
                } else {
                    return false
    ...
    def("text/x-kotlin", {
        ...
        defKeywords: words(
            "class val var object package interface fun"),
    ...
    def("text/x-objectivec", {
        ...
        hooks: {
            ...
            indent: function(_state, ctx, textAfter) {
                if (ctx.type == "statement" && /^@\w/.test(
                        textAfter)) return ctx.indented
            }
        },
        modeProps: {
            fold: "brace"
    ...
    def("text/x-ceylon", {
        ...
        hooks: {
            ...
            '"': function(stream, state) {
                state.tokenize = tokenCeylonString(stream.match(
                    '""') ? "triple" : "single");
                return state.tokenize(stream, state);
            },
            '`': function(stream, state) {
